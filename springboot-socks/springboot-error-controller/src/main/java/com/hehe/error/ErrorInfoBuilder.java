 ...
 *
 * 如果后续想拓展不同种类的错误/异常信息，只需修改ErrorInfoBuilder#getError方法即可，无需修改ErrorController的代码，十分方便
 *
 * @author yizhiwazi
 * @see ErrorInfo
 * @see ErrorProperties
...
// 可有多个@ControlerAdvice注解对应的类,若2个类中都处理接收了某一种异常,则会根据Order的加载顺序,谁先加载就用谁的异常接收处理. 指定多个最后加载的那个才有效
// 用在过滤器中代表该过滤器在众多过滤器中级别最高,过滤时最先执行    默认值为最低等级
@Component
